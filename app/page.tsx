...
const DICT: Record<string, string> = {
  hello: "สวัสดี", world: "โลก", portfolio: "ผลงาน", developer: "นักพัฒนา",
  project: "โปรเจกต์", thank: "ขอบคุณ", you: "คุณ", today: "วันนี้",
  "สวัสดี": "hello", "โลก": "world", "ผลงาน": "portfolio", "นักพัฒนา": "developer",
};
export default function Home() {
  const [input, setInput] = useState("hello portfolio developer");
  const out = useMemo(() => {
    return input.split(/(\s+)/).map((tok) => {
      if (/^\s+$/.test(tok)) return tok;
      const key = tok.toLowerCase();
      return DICT[key] || DICT[tok] || tok;
    }).join("");
  }, [input]);
  return (
    <Shell title="Translation" subtitle="Tiny dictionary-based EN/TH demo — not a production NMT model.">
      <div className="grid gap-4 md:grid-cols-2">
        <textarea className={`${inputClass} min-h-[160px]`} value={input} onChange={(e) => setInput(e.target.value)} />
        <textarea className={`${inputClass} min-h-[160px]`} readOnly value={out} />
      </div>
    </Shell>
  );
}
